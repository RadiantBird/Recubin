...
uint32_t recommendedPhysicsWorkerCount() {
    ...
    if (logicalThreads <= 1) return 1;

    // Box3D recommends performance cores rather than every SMT/E-core thread.
    // Half the reported logical processors is a portable approximation; four
    // workers (including the calling thread) keeps small scenes from paying
    // excessive scheduling overhead.
    return std::clamp((logicalThreads + 1u) / 2u, 2u, 4u);
}

bool sameCFrame(const CFrame& first, const CFrame& second) {
...
bool Box3DPhysicsBackend::init() {
    ...
    definition.workerCount = recommendedPhysicsWorkerCount();
    ...
    RCBN_LOG("Box3D physics initialized with "
             << b3World_GetWorkerCount(m_worldId) << " worker(s)");
...
void Box3DPhysicsBackend::setBodyWorldCFrame(
    ...
    if (entry != m_bodies.end()) {
        entry->synchronizedWorldCFrame =
            worldCFrame * cube.m_compoundLocalOffset;
        entry->hasSynchronizedWorldCFrame = true;
    }
}
...
Vector3 Box3DPhysicsBackend::getAngularVelocity(const BaseCube& cube) const {
    const b3BodyId id = bodyId(cube);
    return B3_IS_NON_NULL(id)
...
b3ShapeId Box3DPhysicsBackend::createCubeShape(
    b3BodyId id, const std::shared_ptr<BaseCube>& cube,
    const CFrame& localFrame, bool sensor) {
    if (!cube || B3_IS_NULL(id)) return b3_nullShapeId;
...
void Box3DPhysicsBackend::recreateActor(const std::shared_ptr<BaseCube>& cube) {
    ...
    if (B3_IS_NULL(replacementShape)) {
...
void Box3DPhysicsBackend::onCubeDestroyed(BaseCube& cube) {
    for (auto it = m_activeTouches.begin(); it != m_activeTouches.end();) {
        if (it->first == &cube || it->second == &cube)
            it = m_activeTouches.erase(it);
        else
            ++it;
    }
    m_touchPairRecords.erase(
        std::remove_if(m_touchPairRecords.begin(), m_touchPairRecords.end(),
            [&](const TouchPairRecord& record) {
                return record.cubes.first == &cube || record.cubes.second == &cube;
            }),
...
void Box3DPhysicsBackend::clearCubes() {
    ...
    m_pendingTouchEnds.clear();
    m_activeTouches.clear();
    m_touchPairRecords.clear();
...
void Box3DPhysicsBackend::syncCubeWithBodyState(
    ...
    if (cube.Anchored) {
        if (entry.sharesBody) {
            ...
            syncCubeWorldCFramePreservingAttachments(cube, memberWorld);
            entry.synchronizedWorldCFrame = memberWorld;
            entry.hasSynchronizedWorldCFrame = true;
            return;
        }
        const CFrame cubeWorld = cube.getWorldCFrame();
        if (entry.hasSynchronizedWorldCFrame &&
            sameCFrame(cubeWorld, entry.synchronizedWorldCFrame)) {
            return;
        }
        ...
        entry.synchronizedWorldCFrame = cubeWorld;
        entry.hasSynchronizedWorldCFrame = true;
        return;
    }
    const CFrame memberWorld = bodyWorldFrame(id) * cube.m_compoundLocalOffset;
    syncCubeWorldCFramePreservingAttachments(cube, memberWorld);
    entry.synchronizedWorldCFrame = memberWorld;
    entry.hasSynchronizedWorldCFrame = true;
}
...
void Box3DPhysicsBackend::applyGyroForces() {
    ...
    for (const ConstraintEntry& entry : m_constraints) {
        ...
        if (!constraint || !constraint->IsA("Gyro")) {
            continue;
        }
        ...
        auto part = gyro->getPart();
        const std::string path = gyro->getFullPath();
        if (!part) {
            reportGyroErrorOnce(
                path + ":unresolved-part",
                "Gyro \"" + path + "\": Part is unresolved during control");
            continue;
        }
        m_gyroErrorKeys.erase(path + ":unresolved-part");
        const std::string partPath = part->getFullPath();

        const b3BodyId id = bodyId(*part);
        if (B3_IS_NULL(id) || !b3Body_IsValid(id)) {
            reportGyroErrorOnce(
                path + ":invalid-body",
                "Gyro \"" + path + "\": Part \"" + partPath +
                    "\" has an invalid Box3D body during control");
            continue;
        }
        m_gyroErrorKeys.erase(path + ":invalid-body");
        if (b3Body_GetType(id) != b3_dynamicBody) {
            reportGyroErrorOnce(
                path + ":non-dynamic-body",
                "Gyro \"" + path + "\": Part \"" + partPath +
                    "\" does not have a dynamic Box3D body");
            continue;
        }
        m_gyroErrorKeys.erase(path + ":non-dynamic-body");

        // @RadiantBird 2026/09/12:
        // A welded member's visible rotation includes its compound offset;
        // controlling only the shared body's rotation targets the wrong frame.
        const Quaternion bodyRotation =
            fromB3Quaternion(b3Body_GetRotation(id));
        const Quaternion memberRotation =
            bodyRotation * part->m_compoundLocalOffset.Rotation;
        ...
        auto computeTorqueForAxis = [&](
            GyroAxis axis,
            b3Vec3 worldAxis
        ) -> float {
            const auto& settings = gyro->getAxisSettings(axis);
            if (!settings.Enabled) {
                return 0.0f;
            }
            ...
            float errorDegrees =
                ...
                Gyro::angleFromRotation(axis, memberRotation);
            ...
            const float maxSpeedRadians =
                settings.MaxAngularSpeed * DEGREES_TO_RADIANS;
            const float desiredAngularVelocity = std::clamp(
                0.5f * RESPONSE_RATE * errorRadians,
                -maxSpeedRadians,
                maxSpeedRadians);
            const float currentAngularVelocity =
                b3Dot(worldAngularVelocity, worldAxis);
            const float angularAcceleration =
                2.0f * RESPONSE_RATE *
                (desiredAngularVelocity - currentAngularVelocity);

            const b3Vec3 inverseInertiaAxis = b3MulMV(
                worldInverseInertia,
                worldAxis);
            const float inverseEffectiveInertia =
                b3Dot(worldAxis, inverseInertiaAxis);
            if (
                !std::isfinite(inverseEffectiveInertia) ||
                inverseEffectiveInertia <= 0.0f
            ) {
                reportGyroErrorOnce(
                    path + ":invalid-inertia:" +
                        std::to_string(static_cast<int>(axis)),
                    "Gyro \"" + path + "\": Part \"" + partPath +
                        "\" has invalid Box3D world inverse inertia "
                        "for an enabled axis");
                return 0.0f;
            }

            m_gyroErrorKeys.erase(
                path + ":invalid-inertia:" +
                std::to_string(static_cast<int>(axis)));
        ...
        appliedTorque.x = computeTorqueForAxis(
            GyroAxis::X,
            {1.0f, 0.0f, 0.0f});
        appliedTorque.y = computeTorqueForAxis(
            GyroAxis::Y,
            {0.0f, 1.0f, 0.0f});
        appliedTorque.z = computeTorqueForAxis(
            GyroAxis::Z,
            {0.0f, 0.0f, 1.0f});
...
void Box3DPhysicsBackend::reportGyroErrorOnce(
    const std::string& key,
    const std::string& message
) {
    if (!m_gyroErrorKeys.insert(key).second) {
        return;
    }
...
void Box3DPhysicsBackend::processContactEvents() {
    const b3SensorEvents sensorEvents = b3World_GetSensorEvents(m_worldId);
    for (int index = 0; index < sensorEvents.beginCount; ++index) {
        const auto& event = sensorEvents.beginEvents[index];
        if (!b3Shape_IsValid(event.sensorShapeId) ||
            !b3Shape_IsValid(event.visitorShapeId)) continue;
        const void* first = b3Shape_GetUserData(event.sensorShapeId);
        const void* second = b3Shape_GetUserData(event.visitorShapeId);
        auto firstCube = resolveContactIdentity(first);
        auto secondCube = resolveContactIdentity(second);
        if (!firstCube || !secondCube || firstCube.get() == secondCube.get()) continue;
        const CubePair pair = normalizePair(firstCube.get(), secondCube.get());
        const bool wasActive = m_activeTouches.contains(pair);
        const bool recordExists = std::any_of(
            m_touchPairRecords.begin(), m_touchPairRecords.end(),
            [&](const TouchPairRecord& value) {
                return value.cubes == pair &&
                    B3_ID_EQUALS(value.sensorShapeId, event.sensorShapeId) &&
                    B3_ID_EQUALS(value.visitorShapeId, event.visitorShapeId);
            });
        if (!recordExists) {
            m_activeTouches.insert(pair);
            m_touchPairRecords.push_back({
                event.sensorShapeId, event.visitorShapeId, pair});
            if (!wasActive)
                m_pendingTouches.emplace_back(firstCube.get(), secondCube.get());
        }
    }
    for (int index = 0; index < sensorEvents.endCount; ++index) {
        const auto& event = sensorEvents.endEvents[index];
        auto record = std::find_if(m_touchPairRecords.begin(), m_touchPairRecords.end(),
            [&](const TouchPairRecord& value) {
                return B3_ID_EQUALS(value.sensorShapeId, event.sensorShapeId) &&
                    B3_ID_EQUALS(value.visitorShapeId, event.visitorShapeId);
            });
        if (record == m_touchPairRecords.end()) continue;
        const CubePair pair = record->cubes;
        m_touchPairRecords.erase(record);
        const bool hasAnotherRecord = std::any_of(
            m_touchPairRecords.begin(), m_touchPairRecords.end(),
            [&](const TouchPairRecord& value) { return value.cubes == pair; });
        if (!hasAnotherRecord && m_activeTouches.erase(pair) != 0) {
            auto firstCube = resolveContactIdentity(pair.first);
            auto secondCube = resolveContactIdentity(pair.second);
            if (firstCube && secondCube)
                m_pendingTouchEnds.emplace_back(firstCube.get(), secondCube.get());
        }
    }
    ...
    for (int index = 0; index < events.beginCount; ++index) {
        ...
        if (!b3Shape_IsValid(event.shapeIdA) || !b3Shape_IsValid(event.shapeIdB)) continue;
...
void Box3DPhysicsBackend::dispatchContactEvents() {
    ...
    auto pendingTouches = std::move(m_pendingTouches);
    m_pendingTouches.clear();
    ...
    m_pendingTouchEnds.clear();
    for (const auto& [firstIdentity, secondIdentity] : pendingEnds) {
        auto first = resolveContactIdentity(firstIdentity);
        auto second = resolveContactIdentity(secondIdentity);
        if (!first || !second || !Physics::s_touchEndCallback) continue;
        Physics::s_touchEndCallback(first.get(), second.get());
    }
}
...
bool Box3DPhysicsBackend::customFilter(
    ...
    if (sensorA || sensorB) {
        if (!first->CanTouch || !second->CanTouch) return false;
    ...
    if (!sameCharacterGroup) return first->CanCollide && second->CanCollide;
    ...
    if (backend->isBallSocketManagedBody(bodyA) &&
        ...
        return first->CanCollide && second->CanCollide;
    }
    return first->CanCollide && second->CanCollide &&
        backend->isBallSocketCollisionPair(bodyA, bodyB);
}
...
void Box3DPhysicsBackend::rebuildAssembly(
    ...
    for (const auto& cube : assembly) {
        ...
        if (found == m_bodies.end())
            ...
            found->synchronizedWorldCFrame = cube->getWorldCFrame();
            found->hasSynchronizedWorldCFrame = true;
        }
    }
...
void Box3DPhysicsBackend::createGyro(const std::shared_ptr<Gyro>& gyro) {
    ...
    m_constraints.push_back({gyro, handle, b3_nullJointId, b3_nullBodyId});
}
...
void Box3DPhysicsBackend::updateConstraint(
    ...
    if (constraint->IsA("Gyro")) {
        ...
        if (!part) {
            reportGyroErrorOnce(
                path + ":unresolved-part",
                "Gyro \"" + path + "\": Part is unresolved while waking constraint");
            return;
        }
        m_gyroErrorKeys.erase(path + ":unresolved-part");
        const std::string partPath = part->getFullPath();
        ...
        if (B3_IS_NULL(id) || !b3Body_IsValid(id)) {
            reportGyroErrorOnce(
                path + ":invalid-body",
                "Gyro \"" + path + "\": Part \"" + partPath +
                    "\" has an invalid Box3D body while waking constraint");
            return;
        }
        m_gyroErrorKeys.erase(path + ":invalid-body");
...
bool isRaycastExcluded(
    ...
    auto parent = instance->Parent.lock();

    while (parent) {
        if (parent.get() == excludeRoot) {
            return true;
        }
...
float box3dRayCallback(
    ...
    auto* context =
        static_cast<RayContext*>(rawContext);
    ...
    if (b3Shape_IsSensor(shapeId)) return -1.0f;
    ...
    if (
        !context->result->hit ||
        fraction * context->maximumDistance <
