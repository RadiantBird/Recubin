...
void registerBuiltinGlobals(Frontend& frontend, GlobalTypes& globals, bool typeCheckForAutocomplete)
{
    ...
    if (TableType* ttv = getMutable<TableType>(getGlobalBinding(globals, "table")))
    {
        if (frontend.getLuauSolverMode() == SolverMode::New)
        {
            ...
            ttv->props["deepcopy"] = makeProperty(idTy, "@luau/global/table.deepcopy");
        }
        else
        {
            ...
            ttv->props["deepcopy"] = makeProperty(makeFunction(arena, std::nullopt, {tabTy}, {tabTy}), "@luau/global/table.deepcopy");
        }
        ...
        attachMagicFunction(*ttv->props["deepcopy"].readTy, std::make_shared<MagicClone>());
