...
- コンテンツブラウザの存在価値がよくわかんないことになってる
  - 要素を右クリックしてFileRefを生成するとかならいいかも
  - assetsディレクトリ固定じゃなくて変更可能にしてもいいと思う
...
- [x] エディター起動時に保存しておきたいことを実装
  - [x] 物理デバッグオプションが有効かどうか
  - [x] カメラの最後の位置
  - [x] 言語設定
  - [x] スナップするか、フィットさせるかどうかのチェックボックスの状態

- [x] パッケージャーのログがスクロールできないのを修正
  - [x] 加えて、ログをコピーできるようにしておく(Luauコンソール/Systemコンソールも同様に)

- [x] スナップ関係のstud/度数の値は小数第三位まで許可するように変更
- [x] 物理タブに前のセッションで追加したインスタンスのボタンを置く

- [x] セカンダリビューポートがなぜ非推奨でバグありなのか一回調べる
  - [x] 触ってみた感じ、常に移動モードになってしまう模様(競合？)
