...
- [x] ツールバー拡張
  - [x] 今まで通り、{再生|選択、移動、...、保存読み込み}の基本バー
  - [x] {Cube, Cylinder, ... Liquid,...}のオブジェクトバー(オブジェクトを追加ボタンは廃止)
  - [x] 地形操作用のバー（プロパティ欄から移動）
  - [x] 物理関係 {物理制約ならべる}バー
  - [x] キャラクター関係
    - [x] Humanoid追加ボタン(インスタンスが選択されてるとき)
    - [x] リグビルダーボタン(デフォルトキャラクターをModelとしてそのまま配置する)

- [x] アタッチメントの位置移動などがコマンド履歴に保存されていない問題の修正

- [x] Particleインスタンス、Weatherインスタンスの保存されるべきプロパティが保存されていない問題の修正

- [ ] Terrainに上下左右前後のブラシを実装する(現状上からしかできないため)

- [x] アニメーションの自動ループをやめ、ループするかどうかを設定できるようにする（エディター、スクリプト両方）
...
- [ ] Decalを改善
  - 8つの画像を自由位置に貼れるのはいいのですが、
    やっぱり6面に、そのまま(長方形/正方形)で貼りたい場合があります。例えば、服のテクスチャとして貼ったり、顔を貼ったり。
    少し拡張していきたいと思っています。
