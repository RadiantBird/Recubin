...
- エディターの改善(1) <-だいぶできた
  - スクリプトの追加をスムーズに
    1. スクリプトの名前を聞く(実装済み) -> `N`とする
    2. Windowsエクスプローラーを開き、ユーザーが選択したフォルダを受け取る
    3. `N`.luauをそのフォルダで新規作成 -> `M`とする
    4. `M`をユーザーのテキストエディタで自動で開くようにする

    また、スクリプトには`Source`プロパティを保存するようにし、編集可能にする(その時はエクスプローラーを開く)
