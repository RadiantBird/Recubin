...
## Todoリスト
(!: 中止|?: 不明|x: 達成済み)
- [x] 移動中にWorkspace移動を連打したりするとWeldされていたCubeが離れていく問題の修正（パッケージされたエンジン固有の問題で、エディターでは発生しない）
...
- [x] 位置の変更などが正しくコマンド履歴に保存されていないことによる、破滅的操作(間違えた->意図しない巻き戻し->混乱)の修正

- [x] 複数選択における複数削除を実装

- [x] コントロールモードがFreeでもキャラクターモデルがRootに追従するようにする(試してみたけどリファクタが必要だった)
