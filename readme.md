...
- エディターの改善(1)
  - トップバーに追加するもの
    - n(float) ユニットごとに移動を設定させる数値ボックスとチェックボックス
    - m(degree) ごとに回転を設定させる数値ボックスとチェックボックス
    - 衝突でフィットする移動モードを有効/無効にするチェックボックス
  - プロパティにRotationとCFrameがないので追加する
  - シーンファイルにCFrameが保存されていないので、回転が破棄される問題の修正
  - DecalにもScriptやSoundのように参照ボタンを追加し、その場で読み込めるようにする
