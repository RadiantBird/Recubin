...
ベイク済みSurfaceGuiは、キャンバスサイズ、自身の可視背景、直接子の描画順・配置・背景・文字・画像・
フォント状態から描画内容署名を計算する。FBO/テクスチャのサイズと署名が前回成功時から変わらなければ、
GL状態変更、FBOクリア、ImGui頂点生成を行わず既存テクスチャを再利用する。`Visible=false`の間は
テクスチャを破棄しないため、内容が変化していなければ再表示時にも再利用できる。
SurfaceGuiがCubeの通常面を上書きする条件は、`Visible`、有効なベイクテクスチャ、自身または直接子の
可視内容がすべて揃うことである。この共通条件をCube個別描画とメインパスのインスタンシング判定が使う。
したがって、透明・非表示・未ベイクのSurfaceGuiが子として残っているだけではCubeを個別描画へ落とさない。
