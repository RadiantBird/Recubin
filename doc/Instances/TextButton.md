...
テキストを表示するクリック可能な画面空間 GUI。`GuiButton` にテキスト内容と文字色を追加する。

## 継承
`Instance` → `ScreenGuiObject` → `GuiButton` → `TextButton`
...
## 依存関係
- `GuiButton`, `Named`
- `Renderer_GUI.cpp`（`renderScreenGui` でのクリック判定・`m_onButtonActivated` コールバック経由）
