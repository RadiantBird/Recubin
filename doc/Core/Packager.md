...
`Config` の文字列値（ゲーム名、出力先、シーンパス、実行ファイルパス）は UTF-8 とする。
Windowsでも日本語を含む `gameName`／`outputDir` をそのままパッケージ名と出力先に使用できる。
パッケージ内の `RecubinEngine.exe` は、起動時に隣接する `startup.yaml` がある場合、そのフォルダを
基準の作業ディレクトリとしてから設定を読み込む（`--scene` を明示した起動では変更しない）。
