...
有効なForceが存在しない固定ステップでは、Force/維持速度処理もBody間の探索を行わず終了する。Forceがある場合も、BodyEntryをBody IDごとに一度だけ集約し、その状態を加算Force適用とMaintainVelocity復元で共有する。従来のBodyごとの全BodyEntry再探索によるO(N²)経路は使用しない。YawForce診断もBody状態から対応インデックスを直接参照し、`m_yawForceDiagnostics`全体をBodyごとに走査しない。CharacterRigのR6 body収集は同じModelについて固定ステップ内で一度だけキャッシュする。物理姿勢の読み戻しでは、子を持たないleaf Cubeに対して`Spatial::setWorldCFrame()`の子孫姿勢保存・ソート経路を使わず、直接CFrameをcommitする。固定ステップ上限を超えた場合の安全装置警告は、継続中に毎フレーム出力せず、発生状態への遷移時だけ出力する。
